...
newtype AppT m a = AppT { runAppT :: ReaderT State m a }
                 deriving ( Functor
                          , Applicative
                          , Monad
                          , MonadIO
...
runAppM :: State -> AppT m a -> m a
runAppM state a = runReaderT (runAppT a) state

runAppM' :: AppT m a -> m a
runAppM' a = runReaderT (runAppT a) State
